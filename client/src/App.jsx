...
function App() {
  ...
  const [aiMessages, setAiMessages] = useState([
    {
      sender: 'ai',
      text: "👋 Hi! I'm **Gemini AI Copilot** powered by Google Gemini.\n\nI can navigate the app for you, calculate sales analytics, update order statuses, manage waitlists, and answer any questions.\n\nHow can I help you today?",
  ...
  const handleAiSend = async (userPromptOverride) => {
    const prompt = (typeof userPromptOverride === 'string' ? userPromptOverride : aiInput).trim();
    if (!prompt) return;
    ...
    if (typeof userPromptOverride !== 'string') setAiInput('');
    setAiLoading(true);
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Text to Speech
        if (aiSpeakerEnabled && window.speechSynthesis) {
          try {
            window.speechSynthesis.cancel();
            const cleanSpeech = (data.reply || '').replace(/[\*\_#`]/g, '');
        ...
        if (data.action === 'export_excel') {
          ...
          fetchOrders(searchTerm, statusFilter, paymentFilter, schoolFilter, dateFrom, dateTo, true);
        }
      } else {
        setAiMessages(prev => [...prev, {
          sender: 'ai',
          text: data.message || 'Failed to process AI request.',
          time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
        }]);
      }
    } catch (err) {
      setAiMessages(prev => [...prev, {
        sender: 'ai',
        text: 'Network error connecting to Gemini AI Assistant.',
        time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      }]);
    } finally {
      setAiLoading(false);
  ...
  const toggleVoiceInput = () => {
    ...
    try {
      ...
      recognition.onstart = () => setAiVoiceActive(true);
      recognition.onend = () => setAiVoiceActive(false);
      recognition.onerror = () => setAiVoiceActive(false);
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div>
                  <h4 style={{ margin: 0, fontSize: '15px', fontWeight: '700', color: '#FFFFFF' }}>Gemini AI Copilot</h4>
                  <p style={{ margin: 0, fontSize: '11px', color: '#94A3B8' }}>Powered by Google Gemini AI</p>
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <button
                  ...
                  style={{
                    background: aiSpeakerEnabled ? 'rgba(99, 102, 241, 0.2)' : 'transparent',
                    border: 'none',
                    color: aiSpeakerEnabled ? '#818CF8' : '#64748B',
                    fontSize: '16px',
                    cursor: 'pointer',
                    padding: '6px',
                    borderRadius: '8px'
            ...
            >
              <button
                ...
                style={{
                  background: aiVoiceActive ? '#EF4444' : 'rgba(51, 65, 85, 0.8)',
                  color: '#FFFFFF',
                  border: 'none',
                  borderRadius: '50%',
                  width: '36px',
                  ...
                  fontSize: '16px',
                  cursor: 'pointer',
                  boxShadow: aiVoiceActive ? '0 0 12px #EF4444' : 'none'
                }}
              >
                🎤
              </button>
              ...
              <input
                type="text"
                placeholder={aiVoiceActive ? "Listening to your voice..." : "Ask Gemini AI Copilot..."}
                value={aiInput}
              ...
              <button
                ...
                style={{
                  ...
                  opacity: aiInput.trim() && !aiLoading ? 1 : 0.5
                }}
              >
                ➔
              </button>
